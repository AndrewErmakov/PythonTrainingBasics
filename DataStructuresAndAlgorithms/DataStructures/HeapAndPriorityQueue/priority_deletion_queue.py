...
# https://informatics.mccme.ru/mod/statements/view3.php?id=1234&chapterid=1168#1
...
class PriorityQueue:
    ...
    def extract_arbitrary_element(self, index):
        """
        The function to delete an item with a specific index
        :param index: index of the item to be deleted
        :return: 1. numerical value of the deleted item
                 2. -1 if there is no element with such an index and deletion is not possible
        """
        if index >= self.size_heap() or index < 0 or self.size_heap() == 0:
            return -1
        elif index == 0:
            return self.extract_max()[1]

        else:
            deleted_element = self.heap_elements[index]
            self.heap_elements[index] = self.heap_elements[0] + 1
            self.sift_up(index)
            self.extract_max()
    ...
    def main_program(self, instruction):
        if instruction[0] == '1':
            ...
            return self.extract_arbitrary_element(int(instruction[1]) - 1)
